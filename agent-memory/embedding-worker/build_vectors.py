...
def point_for_item(source_type: str, item: dict[str, Any], vector: list[float]) -> dict[str, Any]:
    if source_type == "memory":
        item_id = int(item["id"])
        return {
        ...
        for source_type in sources:
            ...
            for batch in batched(items, args.batch_size):
                ...
                for item, vector in zip(batch, vectors):
                    ...
                    out.write(json.dumps(point_for_item(source_type, item, values), ensure_ascii=False, separators=(",", ":")) + "\n")
                    count += 1
