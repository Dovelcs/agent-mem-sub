...
- These low-frequency lifecycle helpers are disabled from default context. Restore the archived `release` skill from `~/.codex/skills.disabled/context-slim-20260426/` if a dedicated release workflow is needed.

8) Collab/multi-agent scenario testing and evidence capture:
- These eval helpers are disabled from default context. Restore the archived `collab-evals` skill if a dedicated eval workflow is needed.
